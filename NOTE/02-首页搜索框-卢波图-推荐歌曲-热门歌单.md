...
- 轮播图 swiper 有固定高度 150px
- iamge 设值 `mode=widthFix` 后，高度由宽度决定。而图片的高度，不等于 swiper 的默认高度，即150px
...
```html
<area-header title="推荐歌曲" bindRightTap="handlleRecommendSongLoadMore" />
```
...
2. 想办法把左边或有便的 margin 抵消掉。
	

## 两种方案

方案一：为 menu-list 设值宽度`100vw`，定位 `left: -24px`
